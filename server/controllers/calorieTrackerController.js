...
const calorieTrackerController = {
  ...
  update: async (req, res) => {
    try {
      ...
      if (!tracker) {
        return res.status(404).json({ success: false, message: "Calorie tracker not found" });
      }
  ...
  delete: async (req, res) => {
    try {
      const tracker = await CalorieTracker.findOneAndDelete({ _id: req.params.id, userId: req.userId });
      if (!tracker) {
        return res.status(404).json({ success: false, message: "Calorie tracker not found" });
      }
